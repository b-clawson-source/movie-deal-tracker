...
class SerpAPISiteSearcher:
    ...
    def search(self, movie_title: str, year: Optional[int] = None) -> List[RetailerResult]:
        ...
        query = f'"{movie_title}" blu-ray ({site_query})'

        if year:
            query = f'"{movie_title}" {year} blu-ray ({site_query})'
...
class RetailerSearcher:
    ...
    def search_all(
        ...
        if self.site_searcher:
            try:
                site_results = self.site_searcher.search(movie_title, year)
...
def search_boutique_retailers(
    ...
    return searcher.search_all(movie_title, year, max_price)
